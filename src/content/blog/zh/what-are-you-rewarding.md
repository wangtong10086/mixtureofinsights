...
这件事有个比 RL 还老的名字:**古德哈特定律(Goodhart's law)**——*当一个度量变成目标,它就不再是
一个好的度量。* 机理是精确的。你的奖励 $r$ 是你写不下来的真实目标 $r^*$ 的一个代理,两者只在你度量
它们的那块区域里相关。优化按其构造,去搜寻最大化 $r$ 的输入——这把策略*拽向那块区域的边缘*,恰恰
是代理与真实目标解耦的地方。你越是猛烈地优化一个固定的代理,就走得越远,你所依赖的相关性也就越糟。
reward hacking 不是你奖励里的 bug;它是把任何不完美的代理优化得足够狠之后的通用后果。所以工程上的
问题从来不是「我的奖励完美吗」(它不完美),而是「策略能走多远,代理才开始撒谎,以及我能否在那之前
把它拦住」。

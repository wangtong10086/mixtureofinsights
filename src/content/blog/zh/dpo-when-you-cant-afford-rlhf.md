---
title: "付不起 RLHF 时,用 DPO"
description: "RLHF 强大但笨重——一个奖励模型、一个在线 rollout 循环、各种不稳定。DPO 用一小部分机器走完大半程,而它之所以能这么做,靠的是一行代数:把奖励模型变成一个闭式目标。这篇给出推导、那个解释它为何有效的梯度,以及那个一如既往的坑——数据。"
date: 2026-06-14
order: 4
...
前两篇用 GRPO 爬一个可验证的奖励。当正确性可检查、且你需要在线探索时,那套机器是对的工具。但很多
对齐不是这样的。「保持人设。」「偏好这种语气。」「别打破第四面墙。」这里没有验证器,也不需要在线
RL——为它上整套 RLHF,是在为一台你不会去开的引擎付钱。**DPO** 是更轻的工具,在一个角色扮演模型上,
它正是合身的那个。它也是后训练里比较漂亮的结果之一,因为它之所以有效,靠的是一段*推导*而非一个
启发式——所以这次我们真的把代数做一遍。

...
在租用的 GPU 上跑后训练任务时，一次运行往往始于 SSH 会话：修改 YAML，再执行启动脚本。笔记本休眠或网络断开后，具体命令和终端里的 `stdout` 可能没有留下完整记录。等到需要验证昨天的 checkpoint，运行它的宿主机已经被回收。
...
在这个拓扑下，控制面持有着一份不可变的期望状态声明。在代码层面，它表现为 [`orbit/core/control`](https://github.com/wangtong10086/mixtureofinsights/blob/main/src/orbit/core/control) 下的 `CoreControlService`。另一端的执行面接收 `JobBundle`，执行节点可以替换。

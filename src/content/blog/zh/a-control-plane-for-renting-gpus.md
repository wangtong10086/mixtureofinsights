...
后训练文章里那些数据引擎、GRPO、奖励模型，最后都得落到某台机器上。问题是那台机器越来越不是你书桌旁
那张稳定的卡，而是一台租来的 GPU：今天开、今晚跑、明天可能就没了。

最开始这只是一些烦人的小事：SSH 会话断了，shell history 找不到，checkpoint 是哪份配置训出来的说不清。
后来我意识到，这不是运维洁癖，而是实验本身的一部分。如果一台机器会消失，那么一次运行就不能只活在那台
机器上。ORBIT 的控制面就是从这个问题长出来的：让训练、评测和数据采集从一次 SSH 副作用，变成一个可以
命名、重试、审计的运行。

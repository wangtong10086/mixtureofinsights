...
有段时间我也很容易把后训练想成优化器选择题：这个任务该上 PPO，还是 GRPO？DPO 会不会更稳？后来真正
卡住的地方却经常不在这里。训练器能跑，loss 也在降，但模型就是学不到你想要的行为；或者评测涨了一点，
下一轮数据一换又掉回去。

这时才会意识到，优化器只负责爬坡，坡本身是数据堆出来的。样本从哪里来，谁负责判它好坏，坏样本怎样被
挡掉，幸存者又怎样被喂回下一轮模型——这些问题比“换哪个 RL 缩写”更早决定结果。ORBIT 里的
`liveweb_teacher_gen.py`、`game_gen.py`、`sft.py` 和 `StaticTraceVerifier`，就是我后来把这条流水线
拆开时留下的几块骨头。

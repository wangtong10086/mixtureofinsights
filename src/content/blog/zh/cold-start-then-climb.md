...
第一次在那个规划任务上直接跑 RL 时，最刺眼的不是坏结果，而是奖励曲线几乎不像曲线。偶尔冒出一点信号，
很快又被噪声吞掉。模型不是完全不会做，它只是太少、太偶然地走到一条可用轨迹上；而策略梯度只能放大它
已经采样到的东西。

后来我对“冷启动”的理解就变得朴素了很多：SFT 不一定要把任务教完，它只需要把策略推到一个 RL 能够探索
的地方。`SwiftConfig` 负责把 SFT/GRPO 配置接起来，`build_ms_swift_dataset` 把幸存轨迹变成训练 JSONL，
rollout 阶段再由 vLLM 采样。文末那个 `+12%` 不是魔法，主要买的是这一步：让 RL 终于有东西可以爬。

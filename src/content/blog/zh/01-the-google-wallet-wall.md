---
title: "Google Wallet 这堵墙"
description: "Play Integrity 拿到了 STRONG,Google Wallet 却仍然拒绝添加卡片。这里讲清原因——是实证而非猜测——以及为什么在一台解锁的设备上,它无法被强行绕过。"
date: 2026-06-09
order: 1
...
一台跑着 LineageOS、解锁了 Bootloader、装着 Magisk root 的小米 13。一整套隐藏栈让它通过了
Play Integrity 的 **BASIC + DEVICE + STRONG**。而 Google Wallet **依然**拒绝添加任何卡片。
这是一个把"到底为什么"彻底查清的故事——以及学会区分"配置缺口"和"一堵墙"。
...
Wallet 向 KeyStore 申请一个**硬件背书的"存储密钥"(storage key)**,然后在**服务端**校验这份
认证。这道校验——而非 Play Integrity——才是那堵墙。
...
- 一个有效、**未被吊销**的 keybox(每个证书序列号都比对过 Google 的 CRL)。
...
每一条路,都终结在同一句 `Device fails attestation`。

...
description: "一次真实的 Android 底层排障：离线 A/B/A、启动时序、限时 eBPF、独立 APK、内核兼容验证，以及一次黑屏留下的教训。最终通过现有 Magisk 和内核的两处修改恢复启动，未新增模块。"
date: 2026-09-14
order: 6
...
**第一道是原模块的信任信息。** 从原内核提取公开的模块验签证书，让新内核保留同一证书，并验证生成的证书逐字节一致。没有使用原私钥，也没有重新签发全部 vendor 模块。运行配置本来就不是强制拒绝所有未签名模块，修复保留了该设置，不能把这次工作描述成额外开启了签名强制校验。[内核模块签名文档](https://docs.kernel.org/5.15/admin-guide/module-signing.html)可用于理解公钥、签名和配置的关系。

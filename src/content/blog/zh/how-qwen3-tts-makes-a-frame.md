...
description: "TTS 不是一张图跑到底，而是一条形状不断变化的小流水线。想把它搬上 OpenVINO，先要知道一帧声音到底由哪些部分拼出来。"
date: 2026-06-10
order: 2
series: "openvino-tts"
reading: "14 分钟"
...
[概览](/zh/blog/when-the-gpu-isnt-an-nvidia/)里说过，TTS 真正麻烦的地方不在“能不能前向一次”，而在它
是一条持续吐声音的循环。刚开始看 Qwen3-TTS 时，我最想弄清楚的不是某个算子怎么导出，而是一个更小的问题：
模型到底怎样造出一帧声音？

顺着这个问题往下看，黑盒会裂成几块：一张 talker 图负责长上下文注意力，一张 cached subcode 图补齐多码本
帧里剩下的部分，后面还有分块流式 decoder。`exporter.py` 和 `build_fastest.py` 里那些边界，都是从这个
拆解里长出来的。
...
而把其余交给一个便宜、被缓存的步骤的架构理由——后面的图拆分,就是沿着这条缝往下走。

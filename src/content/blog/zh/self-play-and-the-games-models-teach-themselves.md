---
title: "自我博弈,与模型自学的游戏"
description: "没有一份「好棋路」的数据集。但在一个有明确胜负的游戏里,你能造出来——让一个强采样器把局打完,按谁赢了来过滤,对局记录就成了策略数据。本系列收官,数据引擎、验证器与涌现策略在这里汇合——落在 Orbit 的 GAME 流水线上。"
date: 2026-06-15
order: 5
...
一个五张牌的不完美信息诈唬游戏里、一轮接一轮、面对一个正在适应的对手的正确着法吗?这你没法手写。但你
能让一个**博弈论采样器**去发现它,再把它的着法蒸馏进模型——这正是 Orbit 的 `GAME` 环境所做的。
...
> 关于框架的一点说明:本文早先的草稿描述过一套以 LLM 为策略的「AI 狼人杀」设置。真正落地的代码是上面
> 这套 OpenSpiel 流水线——用经典博弈论采样器的棋类/牌类游戏——所以这次重写把每一条论断都落在
> `orbit/data/game_*` 上。下面的自我博弈*动力学*(免费验证器、自动课程、非平稳)是通用的,两者都适用;
> 而具体的机器,就是这台 GAME 引擎。
